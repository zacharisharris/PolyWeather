...
export async function GET(req: NextRequest) {
  const timer = createProxyTimer(req, "arbitrage_overview");
  if (!API_BASE) {
  ...
  for (const key of ["city", "force_refresh"]) {
    const value = req.nextUrl.searchParams.get(key);
    if (value != null && value !== "") {
      params.set(key, value);
    }
  }
  const url = `${API_BASE}/api/arbitrage/overview?${params.toString()}`;
  ...
  try {
    return await proxyBackendJsonGet(req, {
      ...
      fetchCache: "no-store",
      publicMessage: "Failed to fetch arbitrage overview",
      includeSupabaseIdentity: true,
      signal: controller.signal,
      timeoutPublicMessage: "Arbitrage overview request timed out",
      timing: timer,
      url,
    });
  } finally {
